...
def wb_nema_iq():
    ...
    parser.add_argument(
        "--dcm_pattern", default="*", help="file pattern for files in the dcm_dir"
    )
    parser.add_argument(
        "--fwhm_mm",
        default=0,
        help="FWHM (mm) of Gaussian filter applied to the input volumes before the analysis",
        type=float,
    )
    parser.add_argument(
        "--radii_mm",
        default=[None],
        help='The radii (mm) of the 6 spheres (seperated by blanks)). If not given this is set the values "18.5 14.0 11.0 8.5 6.5 5.0" are used',
        nargs="+",
    )
    parser.add_argument(
        "--signal",
        default=None,
        help="Fixed signal in [Bq/ml] (or the units of the volume) used when fitting all spheres. If not provided, the fitted value from the biggest sphere is used for all spheres",
        type=float,
    )
    parser.add_argument(
        "--wall_mm",
    ...
    parser.add_argument(
        "--true_act_conc",
        default=None,
        help="True activity concentration in the spheres in [Bq/ml] (or the units of the volume). If not given, it is obtained from the fitted signal of the biggest sphere.",
        type=float,
    )
    parser.add_argument(
        "--output_dir", help="name of the output directory", default=None
    )
    ...
    from .nema_wb import (
        fit_WB_NEMA_sphere_profiles,
        show_WB_NEMA_profiles,
        show_WB_NEMA_recoveries,
    )
    ...
    fitres, sphere_results = fit_WB_NEMA_sphere_profiles(
        vol, voxsize, sameSignal=True, Rfix=Rfix, Sfix=Sfix, dfix=dfix, showBGROI=True
    )
    ...
    fig = show_WB_NEMA_profiles(fitres)
    ...
    fig2 = show_WB_NEMA_recoveries(
        sphere_results, true_act_conc, earlversion=earlversion
    )
