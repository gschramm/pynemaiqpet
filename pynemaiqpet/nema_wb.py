...
def find_background_roi(vol, voxsize, Rcenter=82, edge_margin=28.0, showROI=False):
    ...
    bg_mask = binary_fill_holes(
        median_filter(np.clip(sphere_2d_img, 0, 0.8 * bg), size=7) > 0.5 * bg
    )
    ...
    if showROI:
        ...
        ax.imshow(
            ...
            cmap=py.cm.Greys,
            vmin=0,
            vmax=np.percentile(sphere_2d_img, 99.9),
        )
...
def plot1dprofiles(vol, voxsizes):
...
def NEMASubvols(
    ...
    for bbox in bboxes:
        ...
        slices.append(
            (
                slice(xstart, xstop, None),
                slice(ystart, ystop, None),
                slice(zstart, zstop, None),
            )
        )
...
def findNEMAROIs(vol, voxsizes, R=None, relth=0.25, bgth=0.5):
    ...
    if R is None:
        R = np.array([18.5, 14.0, 11.0, 8.5, 6.5, 5.0])
...
def fit_WB_NEMA_sphere_profiles(
    vol,
    voxsizes,
    sm_fwhm=0,
    margin=9.0,
    ...
    if Rfix == None:
        Rfix = [None] * len(subvols)
    if len(Rfix) < len(subvols):
        Rfix = Rfix + [None] * (len(subvols) - len(Rfix))

    # initial fit to get signal in the biggest sphere
    if (Sfix == None) and (sameSignal == True):
        initfitres = fitspheresubvolume(
            subvols[0],
            voxsizes,
            dfix=dfix,
            Bfix=bg_mean,
    ...
    sphere_results = pd.DataFrame(
        {
            "R": Rs,
            "FWHM": fwhms,
            "signal": Ss,
            "mean_a50": sphere_mean_a50,
            "mean": sphere_mean,
            "max": sphere_max,
...
# -------------------------------------------------------------------------------------------
def show_WB_NEMA_recoveries(
    sphere_results, true_activity, earlcolor="lightgreen", earlversion=2
):
    ...
    fig2, axes2 = py.subplots(1, 2, figsize=(6, 4.0), sharex=True)
