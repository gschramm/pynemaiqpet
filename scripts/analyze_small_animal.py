...
for nifti_file in fnames:
    ...
    vol_aligned = nsa.align_nema_2008_small_animal_iq_phantom(
        vol, voxsize, version=phantom_name
    )

    # generate the ROI label volume
    roi_vol = nsa.nema_2008_small_animal_pet_rois(
        vol_aligned, voxsize, phantom=phantom_name, lp_voxel=lp_voxel
    )
    ...
    vi = pv.ThreeAxisViewer(
        [vol_aligned, vol_aligned], [None, roi_vol**0.1], voxsize=voxsize, ls=""
    )
